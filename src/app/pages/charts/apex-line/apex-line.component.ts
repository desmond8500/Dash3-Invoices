...
@Component({
    ...
    styleUrl: './apex-line.component.scss'
})
...
export class ApexLineComponent {
  ...
  private TICKINTERVAL = 1000; // Adjust as needed
  private XAXISRANGE = 10000; // Define the range for X-axis
  private lastDate: number = new Date().getTime();
  private data = [
    { x: 1488585600000, y: 0 },
    { x: 1488585600000, y: 0 },
    { x: 1488585600000, y: 0 },
  ...
  ngOnInit(): void {
    ...
    */
    ...
    this._basicLineChart('["--in-primary"]');
    this._zoomableTimeseriesChart('["--in-secondary"]');
    this._lineWithDataLabelsChart('["--in-primary", "--in-success"]');
    this._dashedLineChart('["--in-primary", "--in-danger", "--in-success"]');
    this._lineAnnotationsChart('["--in-primary"]');
    this._SyncingLineChart('["--in-primary"]');
    this._Syncingline2Chart('["--in-warning"]');
    this._SyncingAreaChart('["--in-success"]');
    this._brushLineChart('["--in-danger"]');
    this._brushAreaChart('["--in-info"]');
    this._stepLineChart('["--in-success"]');
    this._gradientChart('["--in-success"]');
    this._missingDataChart('["--in-primary", "--in-danger", "--in-success"]');

    this.initializeRealTimeChart('["--in-secondary"]');

    setInterval(() => {
      this.getNewSeries(this.lastDate, { min: 10, max: 90 });
      this.lineChartOptions.series = [{ data: this.data }];
    }, this.TICKINTERVAL);
  }

  private initializeRealTimeChart(colors: any) {
    ...
    this.lineChartOptions = {
      ...
      markers: { size: 0 },
      colors: colors, // Update to your desired color or dynamic color array
  ...
  private getNewSeries(baseval: number, yrange: { min: number; max: number }): void {
    const newDate = baseval + this.TICKINTERVAL;
    this.lastDate = newDate;

    for (let i = 0; i < this.data.length - 10; i++) {
      this.data[i] = { x: newDate - this.XAXISRANGE - this.TICKINTERVAL, y: 0 };
    }
